class PSO():
  ...
  def _create_particles(self):
    ...
    for ind_particle in range(self.num_particles):
      self.particle_curr_pos[ind_particle][0] = (-1)**(bool(random.getrandbits(1))) * \
                                                random.random()*50   # random start point x
      self.particle_curr_pos[ind_particle][1] = (-1)**(bool(random.getrandbits(1))) * \
                                                random.random()*50   # random start point y
  ...
  def _init_global_best_pos(self):
    ...
    self.gbest_pos[0] = random.random()*50
    self.gbest_pos[1] = random.random()*50
  ...
  def pbest_setter(self):
    for particle in range(self.num_particles):
      ...
      if(self.particle_best_val[particle] > benchmark_reward_val):
        #update best value and corresponding position
        self.particle_best_val[particle] = benchmark_reward_val
  ...
  def start_particle_motion(self):
    for particle in range(self.num_particles):
      ...
      particle_curr_velo[particle] = new_velocity # update particle velocity
      particle_curr_pos[particle] = new_position  # update particle position
  ...
  def ps_optimizer(self, num_iterations):
    ...
    while(iter < num_iterations):
      ...
      '''
      Parameter Optimization: SOTA for PSO paramtere optimzation states that
